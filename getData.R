...
dfAll <- data.frame(handNum = integer(0), BBSize = integer(0), JamesPos = character(0), JamesCard1 = character(0),
...
for(f in 1:length(filelist)){
  ...
  dfBig <- data.frame(handNum = integer(0), BBSize = integer(0), JamesPos = character(0), JamesCard1 = character(0),
                      JamesCard2 = character(0), JamesStack = numeric(0), JamesName = character(0),
                      JamesAction = character(0), JamesAmount = integer(0),
                      street = character(0), pot = numeric(0), comCard1 = character(0),
                      comCard2 = character(0), comCard3 = character(0),
  ...
  for(p in 1:length(hands)){
    ...
    handLong <- hands[[p]][grep("PokerStars Hand #",hands[[p]]):length(hands[[p]])]
    handShort <- hands[[p]][1:(grep("PokerStars Hand #",hands[[p]])-2)]
    ...
    df[,c(1:5,17,18,22:25,27,28,32:35,37,38,42:45,47,48,52:55,57,58,62:65)] <- df[1,c(1:5,17,18,22:25,27,28,32:35,37,38,42:45,47,48,52:55,57,58,62:65)]
    
    if(nrow(df) > 1){
      
      options(warn=-1)
      
      df <- hasCards(df, preProcessData2$playerInfo, preFlop$finalTriplets, flop$finalTriplets,
                     turn$finalTriplets)
      
      df <- updateStacks(df, preProcessData2$playerInfo, preFlop$finalTriplets, flop$finalTriplets,
